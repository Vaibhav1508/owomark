              ...
              child: Column(
                children: <Widget>[
                  ...
                  ListTile(
                    ...
                    trailing: Text(total.toString() + ' Rs',
                        style: TextStyle(
                            fontSize: 20,
                            color: Colors.black87,
        ...
        if (data["status"] == "200") {
          ...
          setState(() {
            for (int i = 0; i < cartitem.length; i++) {
              ...
              mtotal = qty * price;
              total = total + mtotal;
              double wt = double.parse(cartitem[i].weight);

              double cwt = double.parse(cartitem[i].cweight);

              mweight = qty * wt;
              weight = weight + mweight;

              mcweight = qty * cwt;
              cweight = cweight + mcweight;

              if (weight > cweight) {
                final_weight = weight;
              } else {
                final_weight = cweight;
              }
            }
          });
        } else {
  ...
  incrementItem(context, String id, String qty) async {
    ...
    response.then((action) async {
      ...
      if (action != null) {
        ...
        if (data["status"] == "200") {
          Fluttertoast.showToast(
              msg: "Item Increased Succeccfully...",
