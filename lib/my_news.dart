...
class MyNews extends StatefulWidget {
  ...
  MyNews({Key key, this.comp_id}) : super(key: key);
...
class _MyNewsState extends State<MyNews> {
          ...
          Row(
            children: <Widget>[
              Row(
                ...
                children: <Widget>[
                 CircleAvatar(child: Icon(Icons.person,color: Colors.white,size: 25,),
  ...
  getNewsFeed(context) async {
    ...
    Future<dynamic> response = apiInterface.getMyNewsFeed(users);
  ...
  likePost(context, String id) async {
   ...
   removeNewsFeed(context, String id) async {
    setState(() {});


    Future<dynamic> response = apiInterface.removeNewsFeed(id);
    ...
    response.then((action) async {
      ...
      if (action != null) {
        Map data = jsonDecode(action.toString());
        if (data["status"] == "200") {
          Fluttertoast.showToast(msg: 'Post removed Successfully');
